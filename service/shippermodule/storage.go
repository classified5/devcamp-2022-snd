...
func (s *storage) DeleteShipper(ctx context.Context, id int64, param m.ShipperRequest) (result m.ShipperResponse, err error) {
	res, err := s.ShipperDB.ExecContext(ctx, deleteShipperQuery,
		id,
	)
...
func (s *storage) UpdateShipper(ctx context.Context, id int64, param m.ShipperRequest) (result m.ShipperResponse, err error) {
	...
	if rowsAffected == 0 {
		log.Println("[ShipperModule][UpdateShipper] ID not found. No rows affected in db")
		return
	}
